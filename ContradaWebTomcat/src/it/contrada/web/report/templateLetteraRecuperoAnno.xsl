				...
				<xsl:for-each select="Elenco/Anagrafica">
					...
					<div style="font-family:calibri;font-size:16px;width:100%;">
						...
						<p align="left" style="line-height=1.5">							
							da un controllo della nostra contabilità risulta, che alla data odierna, per alcuni membri della sua famiglia,
							<xsl:if test="AnnoDA!=AnnoA">
							non sono state ancora da voi pagate le tessere del protettorato dall'anno <xsl:value-of select="AnnoDA" /> all'anno <xsl:value-of select="AnnoA" />.
							</xsl:if>
							<xsl:if test="AnnoDA=AnnoA">
							non sono state ancora da voi pagate le tessere del protettorato per l'anno <xsl:value-of select="AnnoA" />.
							</xsl:if>
							...
							<br />
							<xsl:value-of select="Nominativo" /> - <xsl:value-of select="Quota" />
							</xsl:for-each>
							<br />
							<br />
							...
							<p>
								...
								<br />
								<p>
									Oppure tramite il bolletino postale in allegato:
								</p>
							</p>
